...
class PinterestBot:
    ...
    def read_descs_from_csv(my_csv):
        ...
        try:
            with open(my_csv, gls.read) as rdr:
                reader = csv.reader(rdr, delimiter=",")
                for single_row in reader:
                    list_of_descs.append(single_row)
    ...
    def pin_image(self):
        ...
        self.driver.find_element_by_id('media-upload-input').send_keys(os.getcwd()+self.image_list[randint(0, (len(self.image_list)-1))])
...
if __name__ == "__main__":
    ...
    pn_bot.extract_users_from_dialog("https://www.pinterest.com/technews24h/_community/")

    links_to_follow = pn_bot.read_links_from_csv(gls.user_accounts_csv)

    for single_link in links_to_follow:

        pn_bot.follow_user(single_link)
